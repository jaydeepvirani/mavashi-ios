...
- (void)cs_showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)position {
    ...
    //sandeep
    [toast setFrame:CGRectMake(0, toast.frame.origin.y+50+10, [[UIScreen mainScreen] bounds].size.height, toast.frame.size.height)];
    ...
    [UIView animateWithDuration:[[CSToastManager sharedStyle] fadeDuration]
                     ...
                     animations:^{
                         toast.alpha = 1.0;
                         [toast setFrame:CGRectMake(0, toast.frame.origin.y-50, [[UIScreen mainScreen] bounds].size.height, toast.frame.size.height)];
...
//        messageLabel.textAlignment = style.messageAlignment;
        ...
        CGSize maxSizeMessage = CGSizeMake((self.bounds.size.width * 1) - 26, self.bounds.size.height * 1);
        CGSize expectedSizeMessage = [messageLabel sizeThatFits:maxSizeMessage];
        // UILabel can return a size larger than the max size when the number of lines is 1
        expectedSizeMessage = CGSizeMake(MIN(maxSizeMessage.width, expectedSizeMessage.width), MIN(maxSizeMessage.height, expectedSizeMessage.height));
        
        messageLabel.frame = CGRectMake(13.0, 0.0, [UIScreen mainScreen].bounds.size.width-26, expectedSizeMessage.height);
    }
    ...
    CGSize constrain = CGSizeMake(messageLabel.bounds.size.width, FLT_MAX);
    
    CGSize size = [self text:messageLabel.text sizeWithFont:messageLabel.font constrainedToSize:constrain];
    
//    CGSize size = [messageLabel.text sizeWithFont:messageLabel.font constrainedToSize:constrain lineBreakMode:messageLabel.lineBreakMode];
    
    wrapperHeight = wrapperHeight+(ceil(size.height / messageLabel.font.lineHeight));
    
    imageView = [[UIImageView alloc] init];//WithImage:[UIImage imageNamed:@"login_msg_popup.png"]];
    imageView.contentMode = UIViewContentModeScaleToFill;
    [messageLabel setTextAlignment:NSTextAlignmentCenter];
    if (wrapperHeight < 50)
    {
        imageView.frame = CGRectMake(10, 0.0, [UIScreen mainScreen].bounds.size.width-20, 50);
        [messageLabel setTextAlignment:NSTextAlignmentCenter];
        messageLabel.frame = CGRectMake(13.0, imageView.frame.origin.y+2, messageLabel.frame.size.width, 36);
    }
    else
    {
        imageView.frame = CGRectMake(10, 0.0, [UIScreen mainScreen].bounds.size.width-20, wrapperHeight);
        
        messageLabel.frame = CGRectMake(13.0, imageView.frame.origin.y+2, messageLabel.frame.size.width, messageLabel.frame.size.height);
    }
    
    if (wrapperHeight < 50)
    {
        wrapperView.frame = CGRectMake(0.0, 0.0, wrapperWidth, 50);
    }
    else
    {
